...
def execute_program(args: List[str], mode: TestMode) -> None:
    ...
    def check_output(result: CompletedProcess):
        ...
        if mode == TestMode.STUB:
            ...
            if (
                result.returncode != 0
                or "*** You've passed ALL tests. Congratulations! ***"
                not in stdout
            ):
                raise RuntimeError(error_string)
        else:
...
def get_exec_args(
    file: Path, lang: Language, test_data_dir: Path
) -> List[str]:
...
@click.argument(
    "lang",
    type=click.Choice(
        [Language.CPP.value, Language.JAVA.value, Language.PYTHON.value]
    ),
)
@click.argument(
